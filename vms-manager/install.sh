...
# install.sh
#
# Installs and configures the "vms-manager" service on Debian 12 (systemd),
# running as a non-root user and depending on a PostgreSQL database via Docker.
#
# Prerequisites:
#   • You have compiled "vms-manager" and that binary is in this directory.
...
#        • vms-manager            (compiled Go binary)
...
#   3) Create /etc/vms-manager and copy .env, .env.db, compose.yaml there
#   4) Copy the vms-manager binary into /usr/local/bin and set ownership/permissions
#   5) Ensure Docker is running, then launch or restart the Postgres container via Docker Compose
#   6) Create a systemd unit file at /etc/systemd/system/vms-manager.service
#        • Sets WorkingDirectory=/etc/vms-manager so the binary can load “.env” by filename
...
#   • All files under /etc/vms-manager are owned by root:root with mode 644 (directories 755).
...
echo
echo "============================================"
echo " Installing vms-manager Service as user: ${INSTALL_USER}"
echo "============================================"
echo
...
##########################
# 3) Create /etc/vms-manager and copy over .env, .env.db, compose.yaml
##########################
ETC_DIR="/etc/vms-manager"
...
##########################
# 5) Install the vms-manager binary to /usr/local/bin
##########################
...
##########################
SERVICE_DST="/etc/systemd/system/vms-manager.service"
echo "→ Writing systemd unit → ${SERVICE_DST}"
cat > "${SERVICE_DST}" << EOF
[Unit]
Description=VMS Manager Service
...
ExecStart=/usr/local/bin/vms-manager
Restart=always
RestartSec=5s
...
##########################
# 7) Reload systemd, enable, and start vms-manager.service
##########################
echo "→ Reloading systemd daemon …"
systemctl daemon-reload

echo "→ Enabling vms-manager.service so it starts on boot …"
systemctl enable vms-manager.service

echo "→ Starting vms-manager.service now …"
systemctl restart vms-manager.service
echo

echo "============================================"
echo "  INSTALLATION COMPLETE!"
echo "  • Check status:   sudo systemctl status vms-manager.service"
echo "  • View logs:      sudo journalctl -u vms-manager.service -b"
